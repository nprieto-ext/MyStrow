...
class LibraryPanel(QWidget):
    ...
    def _refresh_custom_effects(self):
        ...
        if custom_effects:
            ...
            empty = QLabel(tr("lt2_no_custom_fx"))
            empty.setStyleSheet(
    ...
    def refresh(self):
        ...
        try:
            if mw and hasattr(mw, '_load_pdf_presets'):
                # Resynchroniser les copies AKAI déjà existantes sur leur preset
                # plan de feu : sans ça, les clips Position posés sur la timeline
                # pointent sur une copie figée au premier usage, et fusionner une
                # nouvelle lyre dans la position ne change rien à la restitution.
                if hasattr(mw, 'sync_pdf_positions_into_akai'):
                    mw.sync_pdf_positions_into_akai()
                _all_pdf = mw._load_pdf_presets() or []
                deja = {p.get("name") for p in pos_presets}
                pdf_presets = [p for p in _all_pdf if p.get("name") not in deja]
        except Exception as e:
        ...
        if not self._sec_gobo._bv.count():
            empty_gobo = QLabel(tr("lt2_no_gobo_wheel"))
...
'''
    ...
    def _build_memory_picker_menu(self, on_select):
        ...
        sl.setContentsMargins(6, 4, 6, 4)
        search_input = QLineEdit()
        search_input.setPlaceholderText(tr("lt2_search_memory"))
        search_input.setClearButtonEnabled(True)
        search_input.setStyleSheet("""
            QLineEdit {
                background: #111; color: #e0e0e0;
    ...
    def _edit_gobo_rotation(self, clip):
        ...
        dlg = QDialog(self)
        dlg.setWindowTitle(tr("lt2_gobo_rotation"))
    ...
    def edit_clip_xfade(self, clip):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle(tr("lt2_crossfade"))
    ...
    def _edit_effect_target_groups(self, clip):
        ...
        dlg = QDialog(self)
        dlg.setWindowTitle(tr("lt2_target_groups"))
